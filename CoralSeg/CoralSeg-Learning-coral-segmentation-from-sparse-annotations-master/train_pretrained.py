...
parser.add_argument("--checkpoint_path", help="checkpoint path", default='./models/deeplab_encoder/') 
...
# Network
output = Network.deeplabv3_plus(input_x, n_classes, training_flag, height, width, max_batch_size)
...
# Different variables
restore_variables = [var for var in tf.global_variables() if 'logits/semantic' not in var.name and 'decoder' not in var.name]   
...
with tf.Session() as sess:
    ...
    ckpt_iou = tf.train.get_checkpoint_state(checkpoint_path + 'iou')
