...
def predict_labels(images, model_options, image_pyramid=None, batch_size=1):
  """Predicts segmentation labels.
  ...
  outputs_to_scales_to_logits = multi_scale_logits(
      ...
      is_training=False,
      fine_tune_batch_norm=False, batch_size=batch_size)
...
def multi_scale_logits(images,
                       ...
                       weight_decay=0.0001,
                       is_training=False,
                       fine_tune_batch_norm=False, batch_size=1):
  ...
  for count, image_scale in enumerate(image_pyramid):
    if image_scale != 1.0:
      ...
      if model_options.crop_size:
        scaled_images.set_shape([1, scaled_height, scaled_width, 3])
    else:
    ...
    outputs_to_logits = _get_logits(
        ...
        fine_tune_batch_norm=fine_tune_batch_norm, batch_size=batch_size)
    ...
    with slim.arg_scope(
      ...
      with slim.arg_scope([slim.batch_norm], **batch_norm_params):
        ...
        if model_options.add_image_level_feature:
          ...
          image_feature.set_shape([batch_size, pool_height, pool_width, depth])
          ...
          for i, name in enumerate(feature_list):
            ...
            for j, feature in enumerate(decoder_features_list):
              ...
              decoder_features_list[j].set_shape(
                  [batch_size, decoder_height, decoder_width, None])
            decoder_depth = 256
